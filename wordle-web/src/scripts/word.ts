  ...
  check(secretWord: string): boolean {
    ...
    for (let i = 0; i < 5; i++) {
      if (guessChars[i] === secretWord[i]) {
        ...
        console.log(`Letter ${this.letters[i].char} is correct but in wrong location.`)
      } else {
        isCorrect = false
        this.letters[i].status = LetterStatus.Wrong
        console.log(`Letter ${this.letters[i].char} is incorrect`)
      }
    }
